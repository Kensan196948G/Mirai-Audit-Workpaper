...
- **日付**: 2026-08-16
- **決定者**: CTO（全権限委譲済み）
- **技術スタック**: Cloudflare Workers (Hono) + D1 (SQLite) + React 19 / Vite / TypeScript + Vitest(node:test)
  - 会社既存標準（mirai-board-audit-governance と同一スタック）に合わせる
...
| GET/POST /api/engagements/:id/remediations | 是正 | 被監査部門・監査役 |
...
- **実装済み（app/ ディレクトリ・検証PASS）**: 認証（セッションCookie + PBKDF2）、RBAC（5ロール×操作権限＋案件隔離）、監査ログ（追記専用）、年度計画（承認フロー＋却下）、個別案件（状態遷移）、証憑依頼（送付・提出・一部受領・受領・差戻し・締切）、調書（版管理・レビュー依頼・差戻し・確定）、指摘・是正（回答・事実確認・是正計画・再確認）、ダッシュボード（自分のタスク）。
- **検証結果**: 型チェック PASS / lint PASS（警告0）/ テスト 48件 PASS（単体+統合）/ ビルド PASS / リンク検証 PASS（84MD + 20HTML）。テストは AC-01〜05（業務フロー）、AC-06（権限外拒否）、CSRF、レート制限、Cookie Secure、状態遷移、bootstrap本番拒否を網羅。
- **セキュリティ強化**: CSRF（Origin/Referer検証）、ログイン・APIレート制限（isolate内）、Cookie Secure属性（HTTPS時）、セキュリティヘッダー（CSP・X-Frame-Options: DENY・nosniff・Referrer-Policy）、CORS全開放の除去、bootstrapの環境変数パスワード化。
- **環境分離**: preview（mirai-audit-workpaper-preview + mvp-DB）／production（mirai-audit-workpaper + main-DB）。productionではbootstrap実行不可。
- **CI/CD**: GitHub Actions（.github/workflows/ci.yml: リンク検証＋型/lint/テスト/ビルド、deploy.yml: preview自動・production手動）。リモート追加後に有効化。
