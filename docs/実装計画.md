...
1. app/ で実装・テスト・ビルド（テスト78件・CI green が前提）
...
> 実施実績（2026-08-16・CHG-034）: preview/MVP は main push で自動デプロイ、本番は workflow_dispatch target=production でデプロイ済み（deployment c0734b06、health db:ok・bootstrap 403 確認）。

---

## 9. 実装状況（2026-08-16 現在・検証済み）

- **実装済み（app/ ディレクトリ・検証PASS）**: 認証（セッションCookie + PBKDF2）、RBAC（5ロール×操作権限＋案件隔離）、監査ログ（追記専用）、年度計画（承認フロー＋却下）、個別案件（状態遷移）、証憑依頼（送付・提出・一部受領・受領・差戻し・締切）、調書（版管理・レビュー依頼・差戻し・確定）、指摘・是正（回答・事実確認・是正計画・進捗・再確認）、ダッシュボード（自分のタスク）、管理者ユーザー管理（作成・更新・無効化/再有効化・パスワード再設定）、被監査部門の情報開示最小化（調書・内部所見・他部門依頼の非開示、R-01対応）。
- **検証結果**: 型チェック PASS / lint PASS（警告0）/ テスト 78件 PASS（単体+統合・health死活含む）/ ビルド PASS / リンク検証 PASS（97MD + 20HTML）。テストは AC-01〜05（業務フロー）、AC-06（権限外拒否）、CSRF、レート制限、Cookie Secure、状態遷移、bootstrap本番拒否、ユーザー管理（UAT-08）、是正進捗、監査ログlimit、セッション無効化、被監査部門の閲覧範囲（R-01）に加え、独立レビュー指摘の回帰（依頼/調書/指摘/是正のアクセス制御・年度グローバル採番・指摘状態機械・シードハッシュ形式）を網羅。
