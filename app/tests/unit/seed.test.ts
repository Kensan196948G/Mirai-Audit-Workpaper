...
const TEST_PASSWORD = "SeedTestPass2026!";
...
describe("unit: seed hash format (P1-8 regression)", () => {
  ...
  test("seed.mjs does not hardcode a default password", () => {
    const src = readFileSync(join(here, "..", "..", "scripts", "seed.mjs"), "utf8");
    assert.ok(src.includes("process.env.MAW_SEED_PASSWORD"), "seed.mjs は環境変数からパスワードを取得する必要があります");
    assert.ok(!src.includes("Mirai@2026pass"), "seed.mjs に既定パスワードをハードコードしない");
  });

  test("seed-style hash with separators is accepted by verifyPassword", async () => {
    const salt = crypto.getRandomValues(new Uint8Array(16));
    const key = await crypto.subtle.importKey("raw", new TextEncoder().encode(TEST_PASSWORD), "PBKDF2", false, ["deriveBits"]);
